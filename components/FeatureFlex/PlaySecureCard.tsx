...
type Ball = { x: number; y: number; r: number; vy: number; spin: number; rot: number; id: number };
...
export default function PlaySecureCard({ className }: { className?: string }) {
  ...
  const metricsRef = useRef({
    cw: 0, ch: 0, trackLeft: 0, trackTop: 0, trackRight: 0, trackBottom: 0,
    trackWidth: 0, trackHeight: 0, halfBtnW: BUTTON_WIDTH / 2, btnH: BUTTON_HEIGHT,
  });
  ...
  useEffect(() => {
    ...
    img.onload = () => { ballImgRef.current = img; ballReadyRef.current = true; };
  }, []);
  ...
  const updateContainerAndTrackMetrics = () => {
    ...
    const yLocked = metricsRef.current.trackTop + metricsRef.current.trackHeight / 2 - metricsRef.current.btnH / 2;
    setBtnY(yLocked);
  };
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(() => { resizeCanvas(); updateContainerAndTrackMetrics(); });
  ...
  const stopDemo = () => {
    ...
    const m = metricsRef.current;
    const center = m.trackLeft + m.trackWidth / 2;
    btnXRef.current = center; setBtnX(center);
  ...
  const onMouseEnter = (e: React.MouseEvent) => { if (!isDemo) startDemo(e.clientX); };
  const onMouseLeave = () => { if (isDemo) stopDemo(); };
